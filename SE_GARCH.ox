...
fExpectation_tGAS(const vTheta, const adFunc, const avScore, const amHessian){
	...
	//and then I have put a negative sign in front of it. This ensures that maximising will
	//effectively search for the value for when the target function is zero.
	adFunc[0] = - (meanc(log(fabs(dALPHA*((dLAMBDA+3)/dLAMBDA) *((dLAMBDA+1)/(dLAMBDA-2)*(1+(vSTD_STUDENT_T.^2)/(dLAMBDA-2)).^(-1).* vSTD_STUDENT_T.^2 ) +dBeta))))^2;
	return 1;
}
...
fExpectation_alGAS(const vTheta, const adFunc, const avScore, const amHessian){
	...
	//and then I have put a negative sign in front of it. This ensures that maximising will
	//effectively search for the value for when the target function is zero.
	adFunc[0] = - (meanc(log(fabs(dALPHA*( sqrt(1-2*dPROB+2*sqr(dPROB))/((1-dPROB)*dPROB)*(dPROB-findicator(vSTD_ALD)).*vSTD_ALD)+dBeta))))^2; 						
	return 1;							  //hier een 2 weggehaald 
}
...
*/
	...
	SetDrawWindow("Stationarity Regions B.T. first set");
	decl iArea = 0;
...
//	print("\nthird set finished \n");
//
//	SetDrawWindow("Stationarity Regions B.T. third set");
//	 iArea = 0;
