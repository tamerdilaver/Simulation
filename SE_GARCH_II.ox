...
fExpectation_GARCH(const vTheta, const adFunc, const avScore, const amHessian){
	...
	adFunc[0] = - (log(meanc(fabs(dALPHA *( vSTD_NORM.^2) + dBeta))))^2; 						
	return 1;
}
...
fExpectation_tGAS(const vTheta, const adFunc, const avScore, const amHessian){
	...
	//and then I have put a negative sign in front of it. This ensures that maximising will
	//effectively search for the value for when the target function is zero.
	adFunc[0] = - (log(meanc(fabs(dALPHA*((dLAMBDA+3)/dLAMBDA) *((dLAMBDA+1)/(dLAMBDA-2)*(1+(vSTD_STUDENT_T.^2)/(dLAMBDA-2)).^(-1).* vSTD_STUDENT_T.^2 ) +dBeta))))^2;
	return 1;
}
...
fExpectation_alGAS(const vTheta, const adFunc, const avScore, const amHessian){
	...
	//and then I have put a negative sign in front of it. This ensures that maximising will
	//effectively search for the value for when the target function is zero.
	adFunc[0] = - (log(meanc(fabs(dALPHA*( 2*sqrt(1-2*dPROB+2*sqr(dPROB))/((1-dPROB)*dPROB)*(dPROB-findicator(vSTD_ALD)).*vSTD_ALD)+dBeta))))^2; 						
	return 1;
}
...
*/
	...
	SetDrawWindow("Stationarity Regions with J.I. II first set");
	decl iArea = 0;
...
//	print("\nsecond set finished \n");
	...
	SetDrawWindow("Stationarity Regions with J.I. II third set");
	 iArea = 0;
	...
	SetDrawWindow("Stationarity Regions with J.I. II fourth set");
	 iArea = 0;
	 mYt = mBeta';    //decl mYt = (mBeta-vNews*(zeros(1,1)~ones(1,2)))';
	//print(vBeta);
	 vX = vNews';
...
//	print("\nfifth set finished \n");
	...
	SetDrawWindow("Stationarity Regions with J.I. II sixth set");
	 iArea = 0;
	 mYt = mBeta';    //decl mYt = (mBeta-vNews*(zeros(1,1)~ones(1,2)))';
	//print(vBeta);
	 vX = vNews';
